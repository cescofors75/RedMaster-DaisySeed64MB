...
/* ─────────────────────────────────────────────────────────────────
 *  FM2Op POLY — wrapper polifónico (mismo interfaz que Synth).
 *  Necesario para "The Bells": las campanas tienen cola larga (~2.6s)
 *  y se solapan; con una sola voz cada nota corta a la anterior.
 *  Asigna voces round-robin robando la más antigua si no hay libres.
 *  Drop-in: NoteOn/NoteOff/Process/IsActive/SetParam/Init idénticos.
 * ───────────────────────────────────────────────────────────────── */
template <int NVOICES = 6>
class PolyT {
public:
    void Init(float sr) {
        for (int i = 0; i < NVOICES; i++) voices_[i].Init(sr);
        tmpl_.Init(sr);
        rr_ = 0;
    }

    void NoteOn(uint8_t midiNote, float velocity = 1.0f) {
        /* Busca voz libre; si no hay, roba la siguiente round-robin */
        int idx = -1;
        for (int i = 0; i < NVOICES; i++) {
            if (!voices_[i].IsActive()) { idx = i; break; }
        }
        if (idx < 0) { idx = rr_; rr_ = (rr_ + 1) % NVOICES; }
        voices_[idx].params = tmpl_.params;    /* timbre actual */
        voices_[idx].NoteOn(midiNote, velocity);
    }

    /* NoteOff global: libera todas (las colas siguen por el release) */
    void NoteOff() {
        for (int i = 0; i < NVOICES; i++) voices_[i].NoteOff();
    }

    float Process() {
        float mix = 0.0f;
        for (int i = 0; i < NVOICES; i++) mix += voices_[i].Process();
        return mix;
    }

    bool IsActive() const {
        for (int i = 0; i < NVOICES; i++) if (voices_[i].IsActive()) return true;
        return false;
    }

    /* SetParam actualiza la plantilla Y las voces sonando (timbre en vivo) */
    void SetParam(uint8_t paramId, float val) {
        tmpl_.SetParam(paramId, val);
        for (int i = 0; i < NVOICES; i++) voices_[i].SetParam(paramId, val);
    }

private:
    Synth voices_[NVOICES];
    Synth tmpl_;          /* mantiene 'params' actual vía su SetParam */
    int   rr_ = 0;
};
